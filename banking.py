...
def bank_menu(user_id,id,password,pos,user_details): #banking menu
    print('\n1. BALANCE ENQUIERY\n2. CASH DEPOSIT\n3. CASH WITHDRAWL\n4. LOGIN MENU')
...
def bal_eq(user_id,id,password,pos,user_details): #balance enquiry
    print(f'\nyour available account balance is: {user_details[id]['balance']}')
    bank_menu(user_id,id,password,pos,user_details)

def deposit(user_id,id,password,pos,user_details):#cash deposit
    amount = int(input('enter the amount you want to deposit: '))
    user_details[id]['balance'] += amount
    print('\nTRANSACTION SUCCESFULL!!\n')
    bank_menu(user_id,id,password,pos,user_details)

def withdrawl(user_id,id,password,pos,user_details):#cash withdrawl
